...
embedding = OpenAIEmbeddings(deployment=OPENAI_EMBEDDING_DEPLOYMENT_NAME,
...
def create_prompts():
    ...
    list = []
    list.append(
        'alcohol | beer | beer bottle | beverage | bottle | can | drink | girl | liquid | party | pink | sip | soda | woman')
    list.append('arcade machine | blow | club | dark | person | laugh | man | nightclub | powder | puff | purple | room | smoke | woman')
    list.append(
        'family | group photo | person | mountain | peak | photo | picture | pose | stand')
    list.append(
        'club | pillar | crowded | girl | person | man | nightclub | party | pose | smile | stand | woman')
